...
extension DriverRegistrationView{
    ...
    private var progressViewStep: some View{
        VStack(alignment: .leading, spacing: 5) {
            Text("Trang \(registrationVM.currentStep)/\(registrationVM.maxStep)")
    ...
    private var nextButton: some View{
        PrimaryButtonView(showLoader: registrationVM.showLoader, title: registrationVM.currentStep != 2 ? "Tiếp theo" : "Chấp nhận đăng kí") {
    ...
    private var step1: some View{
        ZStack{
            VStack(alignment: .leading, spacing: 20) {
                ...
                TextFieldWhithTitle(title: "Họ và tên", lebel: "Họ và tên", text: $registrationVM.fullName)
                TextFieldWhithTitle(title: "Thành phố", lebel: "Thành phố", text: $registrationVM.fullName)
                photoPickerSection
                nextButton
            }
            .padding()
    ...
    private var photoPickerSection: some View{
        VStack(alignment: .leading, spacing: 20) {
            ...
            Button {
                ...
                photoPickerLabel("Giấy phép lái xe", subtitle: "Giấy phép lái xe là một tài liệu chính thức", isComplete: registrationVM.licenseImageData != nil)
            }
            Button {
                registrationVM.selectedImageType = .profile
                registrationVM.showPhotoPicker = true
            } label: {
                photoPickerLabel("Hình ảnh đại diện", subtitle: "Ảnh hồ sơ sẽ hiển thị với hành khách", isComplete: registrationVM.profileImageData != nil)
            }
        }
    }
