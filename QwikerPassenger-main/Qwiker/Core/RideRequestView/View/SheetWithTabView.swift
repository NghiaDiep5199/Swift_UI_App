...
extension SheetWithTabView{
    private var bottomSectionView: some View{
        ...
        return HStack(spacing: 20){
            ...
            PrimaryButtonView(showLoader: false, title: "Yêu cầu \(homeVM.selectedRideType.title)") {
                homeVM.requestRide()
            }
            .withoutAnimation()
            Button {
...
extension SheetWithTabView{
    ...
    private var listSectionView: some View{
        VStack(spacing: 20) {
            Button {
                ...
                listCellView(title: "Góp ý cho tài xế")
            }
            Divider()
            Button {
                
            } label: {
                listCellView(title: "Đặt hàng cho người khác")
            }
        }
        .padding(.horizontal)
    }
    ...
    private func locationLabel(isDestination: Bool = false) -> some View{
        HStack(alignment: .lastTextBaseline) {
            ...
            VStack(alignment: .leading, spacing: 15){
                HStack(spacing: 10) {
                    Text(isDestination ? homeVM.selectedLocation?.title ?? "Điểm đến" : homeVM.userLocation?.title ?? "Vị trí hiện tại")
                        .lineLimit(1)
                        .font(.subheadline.weight(.medium))
