...
extension VerificationView{
    private var title: some View{
        VStack(alignment: .center, spacing: 20) {
            ...
            Text("Một mã đã được gửi đến ").font(.poppinsRegular(size: 14)).foregroundColor(.gray) + Text(authVM.phoneNumber).font(.poppinsMedium(size: 16))
        }
    }
    ...
    private var verifButton: some View{
        PrimaryButtonView(showLoader: authVM.isShowLoader, title: "Xác minh") {
    ...
    private var sendAgainSection: some View{
        HStack {
            if isHiddenTimer{
                ...
                Text("Thêm mã trong \(timeRemaining) giây")
            }
        }
        .font(.footnote)
